...
impl GraphicsCaptureApi {
    ...
    pub fn new<T: GraphicsCaptureApiHandler<Error = E> + Send + 'static, E: Send + Sync + 'static>(
        ...
        if secondary_window_settings != SecondaryWindowSettings::Default && !Self::is_secondary_windows_supported()? {
            eprintln!("[windows-capture] Secondary window settings not supported, using system default");
        }

        if minimum_update_interval_settings != MinimumUpdateIntervalSettings::Default
            && !Self::is_minimum_update_interval_supported()?
        {
            eprintln!("[windows-capture] Minimum update interval not supported, using system default");
        }

        if dirty_region_settings != DirtyRegionSettings::Default && !Self::is_dirty_region_supported()? {
            eprintln!("[windows-capture] Dirty region settings not supported, using system default");
        }
        ...
        if cursor_capture_settings != CursorCaptureSettings::Default {
            if Self::is_cursor_settings_supported()? {
                match cursor_capture_settings {
                    ...
                    CursorCaptureSettings::WithCursor => {
                        if let Err(e) = session.SetIsCursorCaptureEnabled(true) {
                            eprintln!("[windows-capture] Failed to set cursor capture: {:?}", e);
                        }
                    }
                ...
                eprintln!("[windows-capture] Cursor settings not supported, ignoring");
            }
        }
        ...
        if draw_border_settings != DrawBorderSettings::Default {
            if Self::is_border_settings_supported()? {
                ...
                eprintln!("[windows-capture] Border settings not supported, ignoring");
            }
        }
            ...
            if Self::is_secondary_windows_supported()? {
                match secondary_window_settings {
                    ...
                    SecondaryWindowSettings::Include => {
                        if let Err(e) = session.SetIncludeSecondaryWindows(true) {
                            eprintln!("[windows-capture] Failed to set secondary windows: {:?}", e);
                        }
                    }
                ...
                eprintln!("[windows-capture] Secondary windows not supported, ignoring");
            }
        }
            ...
            if Self::is_minimum_update_interval_supported()? {
                match minimum_update_interval_settings {
                    ...
                    MinimumUpdateIntervalSettings::Custom(duration) => {
                        if let Err(e) = session.SetMinUpdateInterval(duration.into()) {
                            eprintln!("[windows-capture] Failed to set minimum update interval: {:?}", e);
                        }
                    }
                }
            } else {
                eprintln!("[windows-capture] Minimum update interval not supported, ignoring");
            }
        }
                ...
                match dirty_region_settings {
                    ...
                    DirtyRegionSettings::ReportOnly => {
                        if let Err(e) = session.SetDirtyRegionMode(GraphicsCaptureDirtyRegionMode::ReportOnly) {
                            eprintln!("[windows-capture] Failed to set dirty region mode: {:?}", e);
                        }
                    }
                    DirtyRegionSettings::ReportAndRender => {
                        if let Err(e) = session.SetDirtyRegionMode(GraphicsCaptureDirtyRegionMode::ReportAndRender) {
                            eprintln!("[windows-capture] Failed to set dirty region mode: {:?}", e);
                        }
                    }
                }
            } else {
                eprintln!("[windows-capture] Dirty region not supported, ignoring");
            }
        }
